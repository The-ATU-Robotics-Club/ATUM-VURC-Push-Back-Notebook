...
\leftsidemember{Layke Bennett}{images/Members/Layke Member.jpg}{I’m not losing my hair; I’m just optimizing my aerodynamics.}{I am a programmer for the team. This is my first year doing collegiate-level robotics, and my fifth year in robotics.}
...
\end{figure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \fbox{\includegraphics[width=0.95\linewidth, height=6.5cm]{images/Field and Field Elements/Max Middle_Goal.jpg}}
...
\end{figure}
    ...
    \begin{subfigure}[b]{0.31\textwidth}
        \centering
        \fbox{\includegraphics[width=0.95\linewidth, height=4.5cm]{images/Field and Field Elements/Red Alliance Loader.jpg}}
    ...
    \begin{subfigure}[b]{0.31\textwidth}
        \centering
        \fbox{\includegraphics[width=0.95\linewidth, height=4.5cm]{images/Field and Field Elements/Blue Alliance Loader.jpg}}
...
\subsection{Autonomous and Driver Strategy}

Now we have discussed all of the rules and requirements for AWP and what we should look for in our strategies. We will now discuss our autonomous and driver routes and why we chose these strategies. 

\begin{itemize}

    \item \textbf{Autonomous Strategies:}
    \\We primarily want to get the control zones in the long goals as those are the most important during driver control, so that we have an initial head-start in defensive play against our opponent.

    \item \textbf{Match Autonomous:}
    \begin{enumerate}[label=\arabic*., leftmargin=*]
        \item
        \begin{minipage}[t]{\linewidth}
            \centering
            \includegraphics[width=0.5\linewidth]{images/Auto Routes/24-inch-Quals Safe.jpg}
            \captionof{figure}{This auto scores one into the bottom Middle goal and then goes to the match loader and intakes all nine. This allows us to have one that is required for AWP, as well as get the control bonus. We stay at the match-loader till we intake all 9 red balls, since we can color-sort the blue balls or vice versa.}
            \label{fig:auto-quals-safe}
        \end{minipage}

        \item
        \begin{minipage}[t]{\linewidth}
            \centering
            \includegraphics[width=0.5\linewidth]{images/Auto Routes/24-inch Elims Control Rush.jpg}
            \captionof{figure}{This is the main auto we will be using in elimination matches for the 24-inch robot. This disregards the pre-load that we place right next to the robot. This allows us to get the control zone as fast as possible so that we can keep it for the rest of autonomous period.}
            \label{fig:auto-quals-safe}
        \end{minipage}
        ...
        \begin{minipage}[t]{\linewidth}
            \centering
            \includegraphics[width=0.5\linewidth]{images/Auto Routes/6-Ball Low Goal.jpg}
            \captionof{figure}{This auto is the auto that we will use when we know we cannot win the control zone faster than our opponent. We score 4 into the top goal and then score 6 into the bottom middle goal so that we can get the control bonus of that and have more balls than our opponent scored.}
            \label{fig:auto-quals-safe}
        \end{minipage}
        ...
        \begin{minipage}[t]{\linewidth}
            \centering
            \includegraphics[width=0.5\linewidth]{images/Auto Routes/24-inch Ball Rush.jpg}
            \captionof{figure}{This auto is another qualifications auto. This one rushes to the middle goal and grabs the 2 balls that are at the Autonomous line. This allows us to score more into the low goal so that we can keep the control zone over our opponent.}
            \label{fig:auto-quals-safe}
        \end{minipage}
        ...
        \begin{minipage}[t]{\linewidth}
            \centering
            \includegraphics[width=0.5\linewidth]{images/Auto Routes/24-inch 1 Ball Rush.jpg}
            \captionof{figure}{This auto is another qualifications auto, it grabs an extra ball from the middle and scores it into the middle with our preload so that we can get the middle goal control bonus over our opponent. Then it goes to the long goal and gets the control zone there. }
            \label{fig:auto-quals-safe}
        \end{minipage}
        ...
        \begin{minipage}[t]{\linewidth}
            \centering
            \includegraphics[width=0.5\linewidth]{images/Auto Routes/15-inch 9+2 No Rush.jpg}
            \captionof{figure}{This Auto pairs with the elims auto with the 24inch to grab the preload that we disregarded. We then go in score it into the top middle goal. We then head to the long goal and score 9 there.}
            \label{fig:auto-quals-safe}
        \end{minipage}
        ...
        \begin{minipage}[t]{\linewidth}
            \centering
            \includegraphics[width=0.5\linewidth]{images/Auto Routes/15-inch 9+2 Middle Top.jpg}
            \captionof{figure}{This auto can be paired with the 6 ball low goal or the 24-inch ball rush since that will give us more balls in the middle goals so we will have more balls scored than our opponent.}
            \label{fig:auto-quals-safe}
        \end{minipage}
        ...
        \begin{minipage}[t]{\linewidth}
            \centering
            \includegraphics[width=0.5\linewidth]{images/Auto Routes/15-inch 9+1 Middle Top.jpg}
            \captionof{figure}{This auto is similar to the 15-inch 9+2 middle top, however it ignores one of the red balls in the neutral zone allowing us into the middle goal faster. It is the safer version of the 9+2 middle top auto.}
            \label{fig:auto-quals-safe}
        \end{minipage}
        ...
        \begin{minipage}[t]{\linewidth}
            \centering
            \includegraphics[width=0.5\linewidth]{images/Auto Routes/15-inch No Pre-Load.jpg}
            \captionof{figure}{This auto disregards the pre-load which will be placed next to the bot instead and won't enter the neutral zone. It will instead go straight for the long goal allowing us to get the control zone before anyone else and hold it. It is the safest and fastest of our auto's.}
            \label{fig:auto-quals-safe}
    ...
    \begin{minipage}[t]{\linewidth}
            \centering
            \includegraphics[width=0.5\linewidth]{images/Auto Routes/Skills Route.jpg}
            \captionof{figure}{For auto skills we have the 24 inch robot go to the other side of the field and then we roughly mirror the motions of the 15 inch robot. This allows us to have a very quick auto skills while it is maxed out. We make sure to clear out every match load station for the bonus as well as remove the balls from the park zone. This gives us extra points. }
            \label{fig:auto-quals-safe}
    ...
    \item \textbf{Driver Strategies:}
    \\ After watching many tournaments ranging from middle school to collegiate level of robotics, we have comprised strategies for both driver and skill matches. Below is a collection of our strategies for focused points of the game. 
    \item Long goal: 
    \\Offensive: We want to get the control zone as soon as possible, once we have gotten the control zone, we want to remove any blocks that are outside of the control zone from the long goal to prevent any defensive plays.\newline
    
    Defensive: We want to load our alliance colored blocks into the long goal, then use our wing to move our blocks into the control zone and clear the rest of their blocks.\newline

    \item Middle goal:
    \\Offensive: We want to take control of the middle goals as quickly as possible since the middle goals are a higher weighted score.\newline
    
    Defensive: We want to descore any of the opponents colored block to set us up for control.\newline

    \item Park Zones: 
    \\ Depending on how or matches go we will rarely park to attempt to hold the control zones and legally prevent the other bots from parking. If we see that we need to park we will park within the last 5 seconds.\newline

    \item Skills: 
    We’re going to create a macro off of our skills autonomous to eliminate the human error that we may face during skills runs.\newline
