...
\begin{figure}[H]
    \centering
    % Placeholder for the image of the rule from the manual
...
\subsection{Top Stage}
    ...
    \subsubsection{Iteration 2}
    The top stage will be drastically different than iteration 1. With this new design, we seek to solve a critical gap problem of iteration 1 through the use of flex wheels. One of the main problems with iteration 1 is that our upper stage would have a tendency to spit balls out accidentally. With the bottom stage sorted out, Custom prints were used on the stop stage in order to achieve a desired geometry for the 1st iteration. 
...
\end{figure}
    ...
    \newpage
    New on cloud mech hat as used in order to have compression when either the ball is at its minimum radius and maximum radius, allowing us to have consistent pressure on the main pivoting. 
...
\subsection{Top Stage}
    ...
    \subsubsection{Iteration 3}
        ...
        Reworked stop stage without 3D printed arms, added 3D printed bearings, and redid rollers for compression along with a continuous curve ramp. 
...
\subsection{Testing Design: Iteration 3}
    ...
    \subsubsection{Problems}
        ...
        \begin{itemize} 
            ...
            \item  The original design for the pivot utilized two screw joints on each side of the pivot using a brass insert to rotate. However, this lead to the 3d print to become reamed out.
        \end{itemize}
    
    \subsubsection{Design Solutions}
        We plan to change the geometry of the curve so we can fit an extra ball, hopefully two. We also plan to add a door mechanism that will make it to where we can shoot out any undesirable colors and also have a front to back option for scoring on the top middle goal.

        A solution was proposed that a polycarbonate guide be added to the stop stage and pivot in order to guide the balls down the stage reducing the chances of offsetting. While the the original pivot worked the reality of a component being able to be fatigued and wore down so easily did not bode well 
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Innovation:} These rollers feature \textbf{print-in-place bearings}. Instead of pressing in commercial bearings, the bearing geometry is printed directly inside the TPU roller during fabrication. This reduces part count and allows for custom bearing sizing that perfectly matches the PLA spoke projection.
    ...
    \end{itemize}
    \item \textbf{Containment Plates (Stainless Steel):}
    \begin{itemize}
        \item Two \textbf{16-gauge stainless steel plates} sandwich the entire assembly.
...
The assembly is compressed and secured using \textbf{M2 bolts}, passing through the steel plates and PLA spokes. This creates a rigid "composite" structure where the steel takes the tension/compression and the printed parts handle the complex geometry.
\newline
...
\textbf{Figure 1: Bearing \& Roller Integration}


\textit{Description:} A close-up of the custom TPU outer roller. The print-in-place bearing is visible in the center. The use of TPU allows the roller to deform slightly over obstacles, improving traction, while the internal geometry remains rigid enough to rotate freely on the PLA spoke.
\newline

\textbf{Figure 2: Final Assembly}


\textit{Description:} The fully assembled Continuous Alternate Wheel. You can see the alternating pattern of the 5 larger TPU rollers (black, outer) and the 5 smaller rollers (grey, inner) retained by the PLA spokes. The stainless steel plates visible in the center hub bind the spokes together, ensuring the wheel can withstand the torque requirements of competitive play.
\newline
...
\section{9/5/25}
    Juan designed the gears used on the drivetrain and printed them, while Cooper finalized the omni wheels.
\newline
...
\section{9/7/25}
    Juan started designing the Senior Design Chassis, fixing the motor caps and mounting them to the chassis. 
\newline

    Cooper printed the omni wheels and the mold for the traction wheel.
\newline

    Hunter casted and assembled the first traction wheel.
\newline
...
\section{9/8/25}
    Juan finalized the design and printed the 5 motor group cap.
\newline
...
\section{9/14/25}
    Kavin started designing and printing custom sprockets to hold rubber bands. 
\newline
    Cooper and Hunter worked on lathing down axles for the Custom Drive Train.
\newline
...
\section{9/19/25}
    The team worked on assembling one side of the Custom Drive Train.
\newline
    Kavin started designing the custom angled braces needed for the first stage of the intake. 
\newline
\begin{figure}[H]
    \centering
...
\section{9/23/25}
    Kavin printed and installed custom angled braces for the first stage of the intake. 
\newline
\begin{figure}[H]
    \centering
...
\end{figure}
\section{9/25/25}
    Collin finished assembling the CAW while Juan designed the cross brace and cutouts for the traction wheels.
\newline
\begin{figure}[H]
    \centering
...
\end{figure}
\section{9/30/25}
    Kavin finished the first prototype on the test bed, while Cooper and Juan continued to make changes to the Senior Design Chassis.
\newline
\begin{figure}[H]
    \centering
...
\section{10/29/25}
    Juan printed a new version of the drive plates and funneling mechanism.
\newline
\begin{figure}[H]
    \centering
...
\section{10/30/25}
    Juan made the first roller for the prototype intake. 
\newline
\begin{figure}[H]
    \centering
...
\section{11/5/25}
    Cooper installed the funnels on the drivetrain while Ryan and Juan designed the back brace.
\newline
\begin{figure}[H]
    \centering
...
\section{11/8/25}
    Juan made our first iteration of the second stage of the intake.
\newline
\begin{figure}[H]
    \centering
...
\section{11/10/25}
    The builders installed the first prototype of the back brace. The team had a little fun with the back brace, they put "ATUM" into the back brace with their signature color.
\newline
...
\section{11/18/25}
    Juan and Ryan worked on the intake for the first prototype; they designed custom motor caps that also support the first stage of the intake. While Ryan installed the motor caps, Juan and Cooper brainstormed the spacing for the secondary intake stages.
\newline
...
\section{12/1/25}
    Juan designed and printed numerous pockets, all slightly different in size to test compression. Having different sizes allows us to quickly change the compression of specific rollers to find the optimal position/compression of each roller.
\newline
...
\section{12/3/25}
Juan made the screw joint mount for the four-bar second stage of the intake.
\begin{figure}[H]
    \centering
...
\newpage
\section{12/5/25}
    Juan tested wrapping the rollers with mesh, while Ryan, Kavin, and Cooper brainstormed a dynamic scoop for the back of the robot. 
\newline
...
\section{12/10/25}
    Juan prototyped different hood styles to hold the balls back while loading it full of balls. Cooper and Hunter worked on a match loading mechanism to make it much easier to retrieve balls from the match loader.
\newline
...
\section{12/14/25}
    Juan and Ryan redesigned the upper stage of the intake, removing it from the robot. Juan also designed a custom cross body axle to support the transition from lower to upper stage and also allow us to pivot across.
\newline
...
\section{12/16/25} 
    Cooper and Hunter worked on lathing the cross body axle, while Juan worked on designing and manufacturing the bottom crossbar. Collin designed a new version of the inner drive plate for the senior design chassis. 
\newline
...
\section{1/8/26}
    Collin redesigned the back cross brace and printed it while Cooper worked on installing the first set of flex wheels on the intake.
\newline
%back brace
\begin{figure}[H]
    \centering
...
\end{figure}
\section{1/12/26}
    We made our first iteration of the descore mechanism.
\begin{figure}[H]
    \centering
    % First Image (Left)
    \begin{subfigure}{0.4\textwidth}
...
\newpage
\section{1/15/26}
    Collin made the motor mount for the first set of flex wheels on the intake.
    \begin{figure}[H]
    \centering
...
\newpage
\section{1/18/26}
    Ryan adjsuted the geometry of the descore arm.
    \begin{figure}[H]
        \centering
...
\section{1/21/26}
    We added the hook to the descore arm, allowing us to test how well this iteration works.
    \begin{figure}[H]
        \centering
...
\newpage
\section{1/22/26}
    We started prototyping a door mech to allow color sorting.
    \begin{figure}[H]
        \centering
...
\section{1/26/26}
    We made a custom door mech that follows the geometry of our intake.
    \begin{figure}[H]
        \centering
...
\newpage
\section{2/1/26}
    We designed an aligner to align the robot against the long goal when descoring.
    \begin{figure}[H]
        \centering
...
\section{2/6/26}
    We mounted the new descore arm along with the aligner to the prototype bot
\begin{figure}[H]
    \centering
    % First Image (Left)
    \begin{subfigure}{0.4\textwidth}
...
\newpage
\section{2/8/26}
    We experimented with a new license plate design idea
    \begin{figure}[H]
        \centering
